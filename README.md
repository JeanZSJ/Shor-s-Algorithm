...
It turns out that not all permutation matrices are equally easy to decompose, and the specific combination of (2,15) happens to hit a numerically unstable edge case in QSD.

### More detailed explanation:
QSD functions resursively. To turn a large unitary operator into basic gates, QSD breaks down a large unitary matrix into smaller pieces, and keep splitting until it reaches gates small enough to implement directly. The splitting is implemented via the Cosine-Sine Decomposition, which uses SVD (which is routed through the `faer`, a Rust library for general purpose linear algebra). However, when it comes down to the bottom of the recursion on 2-qubit unitaries, you have to do something called the Weyl decomposition, which involes Schur decomposition. Schur decomposition is routed through `nalgebra`, another Rust linear algebra library. `nalgebra` could fail on some numerical edge cases, so that's why our (2,5) input failed.

We can use Isometry to work around this problem. The isometry decomposition uses a fundamentally different mathematical approach. Instead of recursively applying CSD and then doing the Weyl decomposition at the bottom, it treats the unitary as an isometry (a map from an n-qubit space into itself) and decomposes it column by column (more explanation on that later). These are direct, closed-form calculations, which does not involve an iterative algorithm that might fail to converge. The tradeoff is that Isometry decomposition generally produces less efficient circuits (with more gates) than QSD.
