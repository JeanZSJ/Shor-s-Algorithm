...
We can use Isometry to work around this problem. Ihe isometry decomposition uses a fundamentally different mathematcial approach. Instead of recursively applying CSD and then doing the Weyl decomposition at the bottom, it treats the unitary as an isometry (a map from an n-qubit space into itself) and decomposes it column by column (more explanation on that later). These are direct, closed-form calculations, which does not invovle an iterative algorithm that might fail to converge. The tradeoff is that Isometry decomposition generally produces less efficient circuits (with more gates) than QSD.
